...
/**
 * 	\defgroup   notesanddocuments     Module NotesAndDocuments
 *  \brief      NotesAndDocuments module descriptor.
 *
 *  \file       htdocs/notesanddocuments/core/modules/modNotesAndDocuments.class.php
 *  \ingroup    notesanddocuments
 *  \brief      Description and activation file for module NotesAndDocuments
 */
include_once DOL_DOCUMENT_ROOT.'/core/modules/DolibarrModules.class.php';

/**
 *  Description and activation class for module NotesAndDocuments
 */
class modNotesAndDocuments extends DolibarrModules
{
	/**
	...
	public function __construct($db)
	{
		...
		$this->rights_class = 'notesanddocuments';
		...
		// Module label (no space allowed), used if translation string 'ModuleNotesAndDocumentsName' not found (NotesAndDocuments is name of module).
		$this->name = preg_replace('/^mod/i', '', get_class($this));
		// Module description, used if translation string 'ModuleNotesAndDocumentsDesc' not found (NotesAndDocuments is name of module).
		$this->description = "NotesAndDocumentsDescription";
		// Used only if file README.md and README-LL.md not found.
		$this->descriptionlong = "NotesAndDocuments description (Long)";
		...
		// Key used in llx_const table to save module status enabled/disabled (where NOTESANDDOCUMENTS is value of property name of module in uppercase)
		...
		$this->module_parts = array(
			...
			'css' => array(
				'/notesanddocuments/css/notesanddocuments.css.php',
			),
			// Set this to relative path of js file if module must load a js on all pages
			'js' => array(
				//   '/notesanddocuments/js/notesanddocuments.js.php',
			),
			// Set here all hooks context managed by module. To find available hook context, make a "grep -r '>initHooks(' *" on source code. You can also set hook context to 'all'
			'hooks' => array('searchform','emailtemplates',
				//   'data' => array(
				//       'hookcontext1',
				//       'hookcontext2',
				//   ),
		...
		// Example: this->dirs = array("/notesanddocuments/temp","/notesanddocuments/subdir");
		$this->dirs = array("/notesanddocuments/temp", "/doctemplates/notesanddocuments");
		// Config pages. Put here list of php page, stored into notesanddocuments/admin directory, to use to setup module.
		$this->config_page_url = array("setup.php@notesanddocuments");
		// Dependencies
		...
		$this->langfiles = array("notesanddocuments@notesanddocuments");
		$this->phpmin = array(5, 5); // Minimum version of PHP required by module
		$this->need_dolibarr_version = array(11, -3); // Minimum version of Dolibarr required by module
		$this->warnings_activation = array(); // Warning to show when we activate module. array('always'='text') or array('FR'='textfr','ES'='textes'...)
		$this->warnings_activation_ext = array(); // Warning to show when we activate an external module. array('always'='text') or array('FR'='textfr','ES'='textes'...)
		//$this->automatic_activation = array('FR'=>'NotesAndDocumentsWasAutomaticallyActivatedBecauseOfYourCountryChoice');
		//$this->always_enabled = true;								// If true, can't be disabled

		// Constants
		// List of particular constants to add when module is enabled (key, 'chaine', value, desc, visible, 'current' or 'allentities', deleteonunactive)
		// Example: $this->const=array(1 => array('NOTESANDDOCUMENTS_MYNEWCONST1', 'chaine', 'myvalue', 'This is a constant to add', 1),
		//                             2 => array('NOTESANDDOCUMENTS_MYNEWCONST2', 'chaine', 'myvalue', 'This is another constant to add', 0, 'current', 1)
		// );
		$this->const = array();
		$r = 0;

		$this->const[$r][0] = "NOTESANDDOCUMENTS_ADDON_PDF_ODT_PATH";
		$this->const[$r][1] = "chaine";
		$this->const[$r][2] = "DOL_DATA_ROOT/doctemplates/notesanddocuments/";
		$this->const[$r][3] = "";
		$this->const[$r][4] = 0;
		$r++;
		...
		if (!isset($conf->notesanddocuments) || !isset($conf->notesanddocuments->enabled)) {
			$conf->notesanddocuments = new stdClass();
			$conf->notesanddocuments->enabled = 0;
		}

		// Array to add new pages in new tabs
		$this->tabs = array();
		// Example:
		// $this->tabs[] = array('data'=>'objecttype:+tabname1:Title1:mylangfile@notesanddocuments:$user->rights->notesanddocuments->read:/notesanddocuments/mynewtab1.php?id=__ID__');  					// To add a new tab identified by code tabname1
		// $this->tabs[] = array('data'=>'objecttype:+tabname2:SUBSTITUTION_Title2:mylangfile@notesanddocuments:$user->rights->othermodule->read:/notesanddocuments/mynewtab2.php?id=__ID__',  	// To add another new tab identified by code tabname2. Label will be result of calling all substitution functions on 'Title2' key.
		...
		'tabrowid'=>array("rowid"), 'tabcond'=>array($conf->notesanddocuments->enabled));
		/* Example:
		$this->dictionaries=array(
			...
			'tabcond'=>array($conf->notesanddocuments->enabled, $conf->notesanddocuments->enabled, $conf->notesanddocuments->enabled)
		);
		*/

		// Boxes/Widgets
		// Add here list of php file(s) stored in notesanddocuments/core/boxes that contains a class to show a widget.
		$this->boxes = array(
			//  0 => array(
			//      'file' => 'notesanddocumentswidget1.php@notesanddocuments',
			//      'note' => 'Widget provided by NotesAndDocuments',
		...
		$this->cronjobs = array(
			...
			//      'unitfrequency' => 3600,
			//      'status' => 0,
			//      'test' => '$conf->notesanddocuments->enabled',
			//      'priority' => 50,
			//  ),
		);
		// Example: $this->cronjobs=array(
		//    0=>array('label'=>'My label', 'jobtype'=>'method', 'class'=>'/dir/class/file.class.php', 'objectname'=>'MyClass', 'method'=>'myMethod', 'parameters'=>'param1, param2', 'comment'=>'Comment', 'frequency'=>2, 'unitfrequency'=>3600, 'status'=>0, 'test'=>'$conf->notesanddocuments->enabled', 'priority'=>50),
        ...
        /* $this->menu[$r++]=array(
            ...
            'fk_menu'=>'fk_mainmenu=notesanddocuments',
            ...
            'langs'=>'notesanddocuments@notesanddocuments',
            'position'=>1100+$r,
            // Define condition to show or hide menu entry. Use '$conf->notesanddocuments->enabled' if entry must be visible if module is enabled. Use '$leftmenu==\'system\'' to show if leftmenu system is selected.
            'enabled'=>'$conf->notesanddocuments->enabled',
            // Use 'perms'=>'$user->rights->notesanddocuments->level1->level2' if you want your menu with a permission rules
            'perms'=>'1',
            'target'=>'',
	...
	public function init($options = '')
	{
		...
		$result = $this->_load_tables('/notesanddocuments/sql/');
		if ($result < 0) return -1; // Do not activate module if error 'not allowed' returned when loading module SQL queries (the _load_table run sql with run_sql with the error allowed parameter set to 'default')

		// Create extrafields during init
		//include_once DOL_DOCUMENT_ROOT.'/core/class/extrafields.class.php';
		//$extrafields = new ExtraFields($this->db);
		//$result1=$extrafields->addExtraField('notesanddocuments_myattr1', "New Attr 1 label", 'boolean', 1,  3, 'thirdparty',   0, 0, '', '', 1, '', 0, 0, '', '', 'notesanddocuments@notesanddocuments', '$conf->notesanddocuments->enabled');
		//$result2=$extrafields->addExtraField('notesanddocuments_myattr2', "New Attr 2 label", 'varchar', 1, 10, 'project',      0, 0, '', '', 1, '', 0, 0, '', '', 'notesanddocuments@notesanddocuments', '$conf->notesanddocuments->enabled');
		//$result3=$extrafields->addExtraField('notesanddocuments_myattr3', "New Attr 3 label", 'varchar', 1, 10, 'bank_account', 0, 0, '', '', 1, '', 0, 0, '', '', 'notesanddocuments@notesanddocuments', '$conf->notesanddocuments->enabled');
		//$result4=$extrafields->addExtraField('notesanddocuments_myattr4', "New Attr 4 label", 'select',  1,  3, 'thirdparty',   0, 1, '', array('options'=>array('code1'=>'Val1','code2'=>'Val2','code3'=>'Val3')), 1,'', 0, 0, '', '', 'notesanddocuments@notesanddocuments', '$conf->notesanddocuments->enabled');
		//$result5=$extrafields->addExtraField('notesanddocuments_myattr5', "New Attr 5 label", 'text',    1, 10, 'user',         0, 0, '', '', 1, '', 0, 0, '', '', 'notesanddocuments@notesanddocuments', '$conf->notesanddocuments->enabled');
