...
/**
 *   	\file       documentnote_card.php
 *		\ingroup    notesanddocuments
 *		\brief      Page to create/edit/view documentnote
 */
...
$contextpage = GETPOST('contextpage', 'aZ') ?GETPOST('contextpage', 'aZ') : 'documentnotecard'; // To manage different context of search
...
$hookmanager->initHooks(array('documentnotecard', 'globalcard')); // Note that conf->hooks_modules contains array
...
$permissiontoadd = $user->rights->notesanddocuments->documentnote->write; // Used by the include of actions_addupdatedelete.inc.php and actions_lineupdown.inc.php
$permissiontodelete = $user->rights->notesanddocuments->documentnote->delete || ($permissiontoadd && isset($object->status) && $object->status == $object::STATUS_DRAFT);
$permissionnote = $user->rights->notesanddocuments->documentnote->write; // Used by the include of actions_setnotes.inc.php
$permissiondellink = $user->rights->notesanddocuments->documentnote->write; // Used by the include of actions_dellink.inc.php
...
if (empty($reshook))
{
	...
	$backurlforlist = dol_buildpath('/notesanddocuments/documentnote_list.php', 1);

	if (empty($backtopage) || ($cancel && empty($id))) {
		if (empty($backtopage) || ($cancel && strpos($backtopage, '__ID__'))) {
			if (empty($id) && (($action != 'add' && $action != 'create') || $cancel)) $backtopage = $backurlforlist;
			else $backtopage = dol_buildpath('/notesanddocuments/documentnote_card.php', 1).'?id='.($id > 0 ? $id : '__ID__');
		}
	}
	$triggermodname = 'NOTESANDDOCUMENTS_DOCUMENTNOTE_MODIFY'; // Name of trigger action code to execute when we modify record
	...
	{
		$object->setValueFrom('fk_soc', GETPOST('fk_soc', 'int'), '', '', 'date', '', $user, 'DOCUMENTNOTE_MODIFY');
	}
	...
	$triggersendname = 'DOCUMENTNOTE_SENTBYMAIL';
	$paramname = 'id';
	$autocopy = 'MAIN_MAIL_AUTOCOPY_DOCUMENTNOTE_TO';
	$trackid = 'documentnote'.$object->id;
...
if ($action == 'create')
{
	...
	print'<td>* Séparer les <strong>mots clés</strong> par une virgule</td>';
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create')))
{
	...
	if ($action != 'presend' && $action != 'editline') {
		...
		if (empty($reshook))
		{
			...
			if ($permissiontoadd)
			{
				print '<a class="butAction" href="'.$_SERVER['PHP_SELF'].'?id='.$object->id.'&socid='.$object->socid.'&action=clone&object=documentnote">'.$langs->trans("ToClone").'</a>'."\n";
			}
	...
	if ($action != 'presend')
	{
		...
		if ($includedocgeneration) {
			...
			$filedir = $conf->notesanddocuments->dir_output.'/'.$objref;
			$urlsource = $_SERVER["PHP_SELF"] . "?id=" . $object->id;
			$genallowed = $user->rights->notesanddocuments->documentnote->read;	// If you can read, you can build the PDF to read content
			$delallowed = $user->rights->notesanddocuments->documentnote->write;	// If you can create/edit, you can remove a file on card
			print $formfile->showdocuments('notesanddocuments:DocumentNote', $objref, $filedir, $urlsource, $genallowed, $delallowed, $object->model_pdf, 1, 0, 0, 28, 0, '', '', '', $langs->defaultlang);
		}

		// Show links to link elements
		$linktoelem = $form->showLinkToObjectBlock($object, null, array('documentnote'));
		...
		$morehtmlright = '<a href="'.dol_buildpath('/notesanddocuments/documentnote_agenda.php', 1).'?id='.$object->id.'">';
